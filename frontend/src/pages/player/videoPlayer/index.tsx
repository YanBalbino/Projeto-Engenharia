...
const SubtitleIcon = () =>(
  <svg width="50px" height="50px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M14 13H18" stroke="#FFFFFF" stroke-width="1.5" stroke-linecap="round"/>
    <path d="M14 16H12.5" stroke="#FFFFFF" stroke-width="1.5" stroke-linecap="round"/>
    <path d="M9.5 13H11.5" stroke="#FFFFFF" stroke-width="1.5" stroke-linecap="round"/>
    <path d="M18 16H16.5" stroke="#FFFFFF" stroke-width="1.5" stroke-linecap="round"/>
    <path d="M6 13H7" stroke="#FFFFFF" stroke-width="1.5" stroke-linecap="round"/>
  </svg>
)
      ...
      {subtititlesModal && (
        <div onMouseLeave={handleSubtitlesModal}  className={"flex flex-col gap-2  absolute bottom-28 mr-6 right-0 bg-slate-600 w-40 h-60"}>
          ...
          <ul className='text-white '>
            <li onClick={() => setSelectedSub(0)} className={`${selectedSub == 0  ? 'bg-slate-500' : ''}  pl-5 hover:cursor-pointer hover:bg-slate-300 `}>Desligadas</li>
            {subtitles.map((subtitle,index)=>{
              return(
                <li onClick={() => setSelectedSub(index+1)} key={index+1} className={`${selectedSub == index+1  ? 'bg-slate-500' : ''}  pl-5 hover:cursor-pointer hover:bg-slate-300 `}>{subtitle}</li>
              )
            })}
          </ul>
        </div>
